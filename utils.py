...
def extract_circuit_metrics(circuit):
    op_counts = circuit.count_ops()
    metrics = {
        'count_ops': op_counts,
        'depth': circuit.depth(),
        'two_qubit_gates': circuit.size(lambda x: x.operation.num_qubits == 2),
        'two_qubit_gate_depth': circuit.depth(lambda x: x.operation.num_qubits == 2),
    }
    return metrics